...
import { internacoesApi } from '@/api/internacoes.api'
import type { Evolucao } from '@/api/internacoes.api'
...
export default function HospitalizationDetailPage() {
  ...
  return (
    <div>
      <PageHeader
        title={`Internação -${internacao.nomePaciente}`}
        description={`Tutor: ${internacao.nomeTutor} | Espécie: ${internacao.nomeEspecie}`}
        actions={
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-card border border-border rounded-xl p-5">
            <h3 className="font-semibold mb-3">Dados da Internação</h3>
            <div className="space-y-2 text-sm">
              <div className="flex items-center gap-2">
                <span className="text-muted-foreground">Status:</span>
                <StatusBadge status={internacao.status} />
              </div>
              <p><span className="text-muted-foreground">Veterinário:</span> {internacao.nomeVeterinario}</p>
              ...
              {internacao.observacoes && (
                <p><span className="text-muted-foreground">Observações:</span> {internacao.observacoes}</p>
              )}
        ...
        <div className="lg:col-span-2">
          <div className="bg-card border border-border rounded-xl p-5">
            <h3 className="font-semibold mb-4">Evoluções ({evolucoes?.length ?? 0})</h3>
                ...
                {evolucoes.map((evol: Evolucao) => (
                  <div key={evol.id} className="border border-border rounded-lg p-4">
                    ...
                    <div className="text-sm space-y-1">
                      {evol.descricao && <p><span className="text-muted-foreground">Descrição:</span> {evol.descricao}</p>}
                      {evol.medicamentos && <p><span className="text-muted-foreground">Medicamentos:</span> {evol.medicamentos}</p>}
                      {evol.alimentacao && <p><span className="text-muted-foreground">Alimentação:</span> {evol.alimentacao}</p>}
                      {evol.observacoes && <p><span className="text-muted-foreground">Observações:</span> {evol.observacoes}</p>}
                    </div>
                  </div>
                ))}
              </div>
      ...
      {showEvolForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-card rounded-xl shadow-lg p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">Nova Evolução</h3>
            <form
              onSubmit={(e) => {
                e.preventDefault()
            ...
            >
              ...
              <div>
                ...
                <textarea placeholder="Descrição da evolução" value={evolForm.descricao} onChange={(e) => setEvolForm({ ...evolForm, descricao: e.target.value })}
              ...
              <div>
                ...
                <input placeholder="Alimentação" value={evolForm.alimentacao} onChange={(e) => setEvolForm({ ...evolForm, alimentacao: e.target.value })}
                  className="w-full px-3 py-2 border border-input rounded-lg text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Observações</label>
                <textarea placeholder="Observações" value={evolForm.observacoes} onChange={(e) => setEvolForm({ ...evolForm, observacoes: e.target.value })}
                  rows={2} className="w-full px-3 py-2 border border-input rounded-lg text-sm" />
              </div>
