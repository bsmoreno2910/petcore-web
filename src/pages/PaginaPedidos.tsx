...
import { pedidosApi, type Pedido } from '@/api/pedidos.api'
            ...
            <div className="bg-card border border-border rounded-xl p-5 space-y-4 sticky top-6">
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                ...
                {order.status !== 'Recebido' && order.status !== 'Cancelado' && (
                  <button onClick={() => setConfirmAction(() => () => cancelMut.mutate(order.id))} className="inline-flex items-center gap-1 px-3 py-1.5 bg-red-500 text-white rounded text-xs"><X size={12} /> Cancelar</button>
                )}
              </div>
            </div>
          ) : (
