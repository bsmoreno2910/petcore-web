...
import { examesApi } from '@/api/exames.api'
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-card border border-border rounded-xl p-5">
            <h3 className="font-semibold mb-3">Dados da Solicitação</h3>
            <div className="space-y-2 text-sm">
              ...
              {exame.indicacaoClinica && (
                <p><span className="text-muted-foreground">Indicação Clínica:</span> {exame.indicacaoClinica}</p>
              )}
              {exame.observacoes && (
                <p><span className="text-muted-foreground">Observações:</span> {exame.observacoes}</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            <div className="space-y-2 text-sm">
              <p><span className="text-muted-foreground">Solicitação:</span> {formatDateTime(exame.dataSolicitacao)}</p>
              {exame.dataColeta && (
                <p><span className="text-muted-foreground">Coleta:</span> {formatDateTime(exame.dataColeta)}</p>
              )}
              {exame.dataConclusao && (
                <p><span className="text-muted-foreground">Conclusão:</span> {formatDateTime(exame.dataConclusao)}</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-5 no-print">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {!['Concluido', 'Cancelado'].includes(exame.status) && (
                <button
                  onClick={() => setConfirmAction(() => () => cancelarMutation.mutate())}
            ...
            {exame.resultado ? (
              <div className="space-y-4">
                ...
                {exame.resultado.valoresReferencia && (
                  <div>
                    <p className="text-sm text-muted-foreground mb-1">Valores de Referência</p>
                    <div className="p-3 bg-secondary rounded-lg text-sm whitespace-pre-wrap">
                      {exame.resultado.valoresReferencia}
                    </div>
                  </div>
              ...
              <p className="text-muted-foreground text-sm">Resultado ainda não disponível.</p>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-card rounded-xl shadow-lg p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            ...
            >
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Conclusão</label>
                <textarea
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Valores de Referência</label>
                <textarea
      ...
      <ConfirmDialog
        ...
        message="Tem certeza que deseja cancelar este exame? Esta ação não pode ser desfeita."
