...
import { produtosApi, type Produto } from '@/api/products.api'

interface ProdutoSearchProps {
  value: { id: string; nome: string } | null
  onChange: (produto: { id: string; nome: string } | null) => void
  error?: string
  disabled?: boolean
}

export function ProdutoSearch({ value, onChange, error, disabled }: ProdutoSearchProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setLoading(true)
      try {
        const data = await produtosApi.listar({ pagina: 1, tamanhoPagina: 8, busca: query })
  ...
  if (value) {
    return (
      <div>
        <label className="block text-sm font-medium mb-1">Produto <span className="text-destructive">*</span></label>
            ...
            <button key={p.id} type="button"
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground truncate">
                  {p.nomeCategoria}{p.apresentacao ? ` - ${p.apresentacao}` : ''}
                </p>
              </div>
            </button>
          ))}
