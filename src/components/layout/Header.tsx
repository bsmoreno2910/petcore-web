  ...
  return (
    <header className="h-16 border-b border-border bg-card flex items-center justify-between px-6">
      <div className="flex items-center gap-3">
        <button onClick={toggleCollapsed} className="p-2 rounded-lg hover:bg-secondary transition-colors"><Menu size={20} /></button>
      </div>

      <div className="relative">
        <button onClick={() => setOpen(!open)} className="flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-secondary transition-colors">
          ...
          <div className="absolute right-0 top-full mt-1 w-72 bg-card border border-border rounded-xl shadow-lg z-50 overflow-hidden">
            ...
            {clinicas.length > 1 && (
              <div className="px-3 py-2 border-b border-border">
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold px-1 mb-1">Clínica</p>
                {clinicas.map(c => (
